...
export default function Tooltip({ tooltip, children, position = 'top' }: TooltipProps) {
  ...
  const arrowClasses = isTop
    ...
    : 'absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 w-2 h-2 bg-black rotate-45';

  return (
    <div className="group relative inline-block">
      {children}
      <div className={`${tooltipClasses} hidden group-hover:block z-10`}>
        <div className="relative bg-black text-white text-md px-2 py-1 rounded whitespace-nowrap">
          {tooltip}
